...
export function LiquidGlassCard({ title, description, imageUrl, imageAlt, className, children }: LiquidGlassCardProps) {
  return (
    <>
      ...
      <div className={cn("relative w-full max-w-md mx-auto", className)}>
        <div className="liquidGlass-wrapper relative flex font-semibold overflow-hidden text-black cursor-pointer shadow-[0_6px_6px_rgba(0,0,0,0.2),0_0_20px_rgba(0,0,0,0.1)] transition-all duration-[400ms] ease-[cubic-bezier(0.175,0.885,0.32,2.2)] rounded-2xl md:rounded-3xl p-2 hover:p-3 hover:scale-[1.02] hover:shadow-[0_12px_12px_rgba(0,0,0,0.3),0_0_40px_rgba(0,0,0,0.2)]">
          ...
          <div className="liquidGlass-effect absolute z-0 inset-0 [backdrop-filter:blur(3px)] [filter:url(#glass-distortion-card)] overflow-hidden [isolation:isolate] rounded-2xl md:rounded-3xl" />
          ...
          {/* Content Layer */}
          <div className="liquidGlass-text z-[3] relative flex h-full flex-col overflow-hidden rounded-xl w-full">
            ...
            <div className="flex flex-1 flex-col justify-between gap-4 p-6 bg-white/90 backdrop-blur-sm">
              <div className="space-y-3">
                <h3 className="font-sans text-xl font-semibold text-balance text-black md:text-2xl">{title}</h3>
                <p className="font-sans text-sm text-black/70 md:text-base">{description}</p>
              </div>
